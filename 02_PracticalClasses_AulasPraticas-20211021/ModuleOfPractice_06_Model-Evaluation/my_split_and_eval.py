...
def load_dataset( fileName, featureName=None, func_datasetLoader=None ):
  ...
  if func_datasetLoader: return func_datasetLoader()
...
def fold_split( k_folds, seed=None ):
  ...
  tt_split_indexes = KFold(k_folds, shuffle=True, random_state=seed) # <your-code-here>
  return tt_split_indexes
...
def stratified_fold_split( k_folds, seed=None ):
  ...
  # - shuffle=True, so dataset is shuffled (random split) before building the folds
  # - datasets preserve the percentage of samples for each class
  tt_split_indexes = StratifiedKFold(k_folds, shuffle=True, random_state=seed) # <your-code-here>
  return tt_split_indexes
...
def repeated_fold_split( k_folds, n_repeat, seed=None ):
  ...
  tt_split_indexes = RepeatedStratifiedKFold(k_folds, random_state=seed) # <your-code-here>
  return tt_split_indexes
...
def repeated_stratified_fold_split( k_folds, n_repeat, seed=None ):
  ...
  tt_split_indexes = RepeatedStratifiedKFold(k_folds, random_state=seed,n_splits=n_repeat) # <your-code-here>
  return tt_split_indexes
...
def leave_one_out():
  ...
  tt_split_indexes = LeaveOneOut() # <your-code-here>
  return tt_split_indexes
...
def leave_p_out( p ):
  ...
  tt_split_indexes = LeavePOut( p ) # <your-code-here>
  return tt_split_indexes
...
def bootstrap_split_once( seed=None ):
  tt_split_indexes = MyBootstrapSplitOnce(seed) # <your-code-here>
  return tt_split_indexes
...
def bootstrap_split_repeated( n_repeat, seed=None ):
  tt_split_indexes = MyBootstrapSplitRepeated(n_repeat, seed) # <your-code-here>
  return tt_split_indexes
...
def score_recipe( classifier, X, y, tt_split_indexes,
    ...
    for ( train_index, test_index ) in tt_split_indexes.split( X, y ):
      ...
      y_predict = classifier.predict(X_test) # <your-code-here>

      # score the model using y_test (expected) and y_predict (predicted by model)
      score = f_score( y_test, y_predict) # <your-code-here>
...
# <your-code-here>
func_datasetLoader = None#simple_dataset # None (if we want to load the "fileName)
